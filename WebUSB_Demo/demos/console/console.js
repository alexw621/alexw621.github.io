  ...
  document.addEventListener('DOMContentLoaded', event => {
    ...
    function connect() {
      ...
      port.connect().then(() => {
        ...
        t.io.println('Connection error (Call from connect): ' + error);
      });
    };
    ...
    connectButton.addEventListener('click', function() {
      if (port) {
        ...
        serial.requestPort().then(selectedPort => {
          ...
          t.io.println('Connection error (Call from connectButton.addEventListener): ' + error);
        });
      }
    });
    ...
    serial.getPorts().then(ports => {
      if (ports.length == 0) {
        t.io.println('No devices found. (ports.length == 0)');
      } else {
        port = ports[0];
        connect();
      }
